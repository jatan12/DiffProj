...
class batch_opt_nonhol():
	...
	def __init__(self):
		...
		self.ellite_num = 50
		self.ellite_num_projection = 300
	...
	# @partial(jit, static_argnums=(0,))	
	# def initial_alpha_d_obs(self, x_guess, y_guess, x_obs, y_obs):
	...
	# 	# print(linalg.norm(prob_data.alpha_obs))        
	...
	# 	d_temp = c2_d/c1_d
	# 	d_obs_temp = jnp.maximum(jnp.ones((self.num_batch,  self.num*self.num_obs   )), d_temp   )
	# 	d_obs = d_obs_temp

	# 	return alpha_obs, d_obs		
	...
	def compute_cost(self, y_ellite_projection, res_ellite_projection, xdot_ellite_projection, ydot_ellite_projection, xddot_ellite_projection, yddot_ellite_projection, y_lane_lb, y_lane_ub, v_des, steering_ellite_projection, d_v_ellite_projection):
		...
		cost_batch = res_ellite_projection+0.001*jnp.linalg.norm(temp_vx, axis = 1)+jnp.linalg.norm(temp_steering, axis = 1)+cost_steering+cost_steering_vel+heading_penalty+0.001*jnp.linalg.norm(y_ellite_projection-(y_lane_ub+y_lane_lb)/2, axis = 1  )
	...
	@partial(jit, static_argnums=(0, ))	
	def compute_projection(self, initial_state, lamda_x, lamda_y, x_obs, y_obs, x_guess, y_guess, c_x_bar, c_y_bar, y_lane_lb, y_lane_ub, alpha_a, d_a, alpha_v, d_v, s_lane):
	...
	@partial(jit, static_argnums=(0, ))
	def compute_ellite_samples(self, cost_batch, c_x_ellite_projection, c_y_ellite_projection):
		...
		return c_x_ellite, c_y_ellite, idx_ellite
	...
	@partial(jit, static_argnums=(0, ))
	def compute_shifted_samples(self, key, c_x_ellite, c_y_ellite, i, x_obs, y_obs, y_lane_lb, y_lane_ub, cost_batch, c_mean_prev, c_cov_prev, idx_ellite):
		...
		# c_cov = temp*jnp.cov(c_ellite.T)+0.01*jnp.identity(2*self.nvar)
		...
		c_mean = (1-self.alpha_mean)*c_mean_prev + self.alpha_mean*(jnp.sum( (c_ellite * w[:,jnp.newaxis]) , axis= 0)/ sum_w)
		diffs = (c_ellite - c_mean)
		prod_result = self.vec_product(diffs, w)
		c_cov = (1-self.alpha_cov)*c_cov_prev + self.alpha_cov*(jnp.sum( prod_result , axis = 0)/jnp.sum(w, axis = 0)) + 0.01*jnp.identity(2*self.nvar)
		...
		x_guess_temp = jnp.dot(self.P_jax, c_x_ellite_shift_temp.T).T 
		y_guess_temp = jnp.dot(self.P_jax, c_y_ellite_shift_temp.T).T 
		y_guess_temp = jnp.clip(y_guess_temp, y_lane_lb*jnp.ones((self.initial_up_sampling*self.num_batch, self.num  )), y_lane_ub*jnp.ones((self.initial_up_sampling*self.num_batch, self.num  ))   )
		...
		c_x_bar = jnp.vstack(( c_x_ellite, c_x_ellite_shift  ))
		c_y_bar = jnp.vstack(( c_y_ellite, c_y_ellite_shift  ))

		x_guess = jnp.dot(self.P_jax, c_x_bar.T).T 
		y_guess = jnp.dot(self.P_jax, c_y_bar.T).T 


		return c_x_ellite_shift, c_y_ellite_shift, x_guess, y_guess, c_x_bar, c_y_bar, c_mean, c_cov, cost_batch_temp
	...
	def compute_cem(self, initial_state, lamda_x, lamda_y, x_obs, y_obs, x_guess, y_guess, c_x_bar, c_y_bar, y_lane_lb, y_lane_ub,  alpha_a, d_a, alpha_v, d_v, v_des, c_mean, c_cov, s_lane):
		...
		for i in range(0, self.maxiter_cem):
			...
			# for i in range(0, 3):
			c_x, c_y, x, y, xdot, ydot, xddot, yddot, alpha_obs, d_obs, alpha_a, d_a, lamda_x, lamda_y, res_norm_batch, alpha_v, d_v, curvature, steering, s_lane = self.compute_projection(initial_state, lamda_x, lamda_y, x_obs, y_obs, x_guess, y_guess, c_x_bar, c_y_bar, y_lane_lb, y_lane_ub, alpha_a, d_a, alpha_v, d_v, s_lane)
			...
			c_x_ellite, c_y_ellite, idx_ellite = self.compute_ellite_samples(cost_batch, c_x_ellite_projection, c_y_ellite_projection)
			

			key, subkey = random.split(self.key)

			c_x_ellite_shift, c_y_ellite_shift, x_guess, y_guess, c_x_bar, c_y_bar, c_mean, c_cov, cost_batch_temp = self.compute_shifted_samples(key, c_x_ellite, c_y_ellite, i, x_obs, y_obs, y_lane_lb, y_lane_ub, cost_batch, c_mean_prev, c_cov_prev, idx_ellite)
