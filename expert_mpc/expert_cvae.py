...
class batch_opt_nonhol():
	...
	def __init__(self, Wandb, BN, inp_mean, inp_std, maxiter_proj):
		...
		self.ellite_num = 5 # 0.4 of num_projection (10, 20, 30, 40)
		self.ellite_num_projection = 10 # 0.1 of num_batch (25, 50, 75, 100)
		...
		self.maxiter = maxiter_proj # Projection 100
		self.maxiter_cem = 1 # CEM
	...
	def compute_cost(self, res_ellite_projection, xdot_ellite_projection, ydot_ellite_projection, v_des, steering_ellite_projection, velocity_scaling):
		...
		cost_batch = res_ellite_projection+velocity_scaling*jnp.linalg.norm(xdot_ellite_projection-v_des, axis = 1)#+cost_steering+cost_steering_vel+heading_penalty
	...
	@partial(jit, static_argnums=(0, ))	
	def compute_cem_nn(self, inp, initial_state, x_obs, y_obs, y_lane_lb, y_lane_ub, alpha_a, d_a, alpha_v, d_v, v_des, s_lane, mean_param, cov_param):
		...
		batch_inp_norm = jnp.vstack([inp_norm] * (self.decode_batch_size))

		# z ~ N(0, 1)
		z = jax.random.normal(key, ((self.decode_batch_size), self.z_dim)) 
		batch_inputs = jnp.concatenate([z, batch_inp_norm], axis = 1)

		# Generate behavioral params and lamda's from the decoder
		neural_output_batch = self.jax_decoder(batch_inputs)
		behavioral_params = neural_output_batch[:, 0:8]
		behavioral_params = behavioral_params.at[:, 0:4].set(jax.nn.sigmoid(behavioral_params[:, 0:4]) * 40. + 0.3)
		lamda_x = neural_output_batch[:, 8:19]
		lamda_y = neural_output_batch[:, 19:]
		...
		def lax_cem(carry,idx):
			...
			lamda_x, lamda_y, alpha_a, d_a, alpha_v, d_v, s_lane, behavioral_params, neural_output_ellite, mean_param, cov_param = carry 
			
			mean_param_prev = mean_param
			
			cov_param_prev = cov_param
			
			c_x_bar, c_y_bar = self.compute_x_guess(initial_state, behavioral_params)
			
			c_x, c_y, x, y, xdot, ydot, xddot, yddot, alpha_obs, d_obs, alpha_a, d_a, lamda_x, lamda_y, res_norm_batch, alpha_v, d_v, curvature, steering, s_lane = self.compute_projection(initial_state, lamda_x, lamda_y, x_obs, y_obs, c_x_bar, c_y_bar, y_lane_lb, y_lane_ub, alpha_a, d_a, alpha_v, d_v, s_lane, behavioral_params)
			...
			neural_output_projection = behavioral_params[idx_ellite_projection[0:self.ellite_num_projection]]
			...
			mean_param, cov_param, behavioral_params, cost_batch_temp = self.compute_shifted_samples(key, neural_output_ellite, cost_batch, idx_ellite, mean_param_prev, cov_param_prev)

			idx_min = jnp.argmin(cost_batch_temp)

			return (lamda_x, lamda_y, alpha_a, d_a, alpha_v, d_v, s_lane, behavioral_params, neural_output_ellite,mean_param,cov_param),(c_x_ellite_projection[idx_min],c_y_ellite_projection[idx_min])
		
		carry_init = (lamda_x, lamda_y, alpha_a, d_a, alpha_v, d_v, s_lane, behavioral_params, jnp.zeros((self.ellite_num, 8)), mean_param, cov_param)
		carry_final, result = lax.scan(lax_cem, carry_init,jnp.arange(self.maxiter_cem))
