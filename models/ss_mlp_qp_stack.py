...
class SSNet(nn.Module):
	def __init__(self, P, Pdot, Pddot, mlp, num_batch, inp_mean, inp_std):
	...
	# pi_omega maps observations to optimal trajectories y*
	def decode(self, x, init_state_ego, x_obs_traj, y_obs_traj, y_ub, y_lb):
     
		# Predicting Behavioral Inputs
		y = self.mlp(x)
		...
		return primal_sol, self.weight_aug * res_norm_batch + self.vel_scale * vel_pen # 0.001

	# Self-supervised Loss
	def ss_loss(self, res_norm_batch):

		# Aug loss
		aug_loss = 0.5 * torch.mean(res_norm_batch)
  
		return aug_loss

	# Forward Pass
	def forward(self, inp, init_state_ego):
		...
		# Decode y
		y_star, res_norm_batch = self.decode(inp_norm, init_state_ego, x_obs_traj, y_obs_traj, y_ub, y_lb)		
	
		return y_star, res_norm_batch