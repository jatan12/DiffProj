...
from highway_env.vehicle.planning_controller import PlanningVehicle
...
class PlanningAction(ActionType):
    """
    Planning coefficients action based on Bernstein polynomial
    """
    ...
    def space(self) -> spaces.Box:
        return spaces.Box(-1000, 1000, shape=(self.size,), dtype=np.float32)
...
def action_factory(env: 'AbstractEnv', config: dict) -> ActionType:
    ...
    elif config["type"] == "PlanningAction":
        return PlanningAction(env, **config)
    else:
