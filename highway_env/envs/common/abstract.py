...
class AbstractEnv(gym.Env):
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, dict]:
        ...
        return obs.flatten(), reward, terminal, info

    def _obs(self, obs):
        last_obs_idx = np.argmax(np.abs(obs[:,0 ]))
        if last_obs_idx == 0:
            obs[last_obs_idx + 1:, :] = np.array([obs[0,0] - 200, obs[0,1], 0, 0, 0])
        elif last_obs_idx < obs.shape[0]-1:
            obs[last_obs_idx+1:, :] = obs[last_obs_idx, :]
        str1 = "0" # 'a' , '0'
        str2 = "1"  # 'b', '1'
        maxy = self.road.network.graph[str1][str2][-1].start[1] + 1
        y_ub = maxy - obs[0, 1]
        y_lb = -maxy - obs[0, 1]
        obs[:, 0:2] = obs[:, 0:2] - obs[0, 0:2]
        obs[0, 0] = y_ub
        obs[0, 1] = y_lb
