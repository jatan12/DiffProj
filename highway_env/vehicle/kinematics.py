...
class Vehicle(RoadObject):
    ...
    def step(self, dt: float) -> None:
        ...
        """
        self.clip_actions()
        self.delta_f = self.action['steering']
        self.acc = self.action['acceleration']
        beta = 0#np.arctan(1 / 2 * np.tan(delta_f))
        ...
        if self.impact is not None:
            ...
            self.crashed = True
            self.impact = None
        r = self.speed * np.tan(self.delta_f) / (self.LENGTH / 2)
        self.heading += r * dt
        self.speed += self.acc * dt

        self.ax = self.acc * np.cos(self.heading) - self.speed * np.sin(self.heading) * r
        self.ay = self.acc * np.sin(self.heading) + self.speed * np.cos(self.heading) * r
